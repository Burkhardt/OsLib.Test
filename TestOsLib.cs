...
namespace OsLib.Tests
{
	...
	public class RaiFileTest
	{
		...
		public void TestRaiPath()
		{
			...
			p /= "test";
			Assert.Equal(p.Path, Os.NormSeperator("/usr/bin/test/"));
			var q = p / "subtest";
			Assert.Equal(q.Path, Os.NormSeperator("/usr/bin/test/subtest/"));
			var path = q.ToString();
			Assert.Equal(path, Os.NormSeperator("/usr/bin/test/subtest/"));
			Assert.Equal(q.Path, q.ToString());
			// Assert.Equal(q.Path, (string)q); // don't use this - .NET has the convention .ToString() instead
		}
		[Fact]
		...
		public void TestCsvFile()
		{
			...
			Assert.True(csv.Exists(), $"TestCsvFile cannot run without a valid csv file in place: {csv.FullName}");
			int count = csv.Read();
			var item = csv[1000]; //was: var item = csv.Item(1000);
			#region test for expected CONLEYde content
			string MerchantProductNumber = item[nameof(MerchantProductNumber)];
			Assert.Equal("6726070", MerchantProductNumber);
			string ProductManufacturerBrand = item[nameof(ProductManufacturerBrand)];
			Assert.Equal("CONLEYS BLUE", ProductManufacturerBrand);
			string ProductName = item[nameof(ProductName)];
			Assert.Equal("Sweatshirt, Kapuze", ProductName);
			var FieldNames = csv.FieldNames();
			Assert.Equal("SizeStockAmount", FieldNames[24]);
			Assert.Equal("ProductName", FieldNames[1]);
			#endregion
		}
		/// <summary>
